...
"""Simulated annealing with 60 threads. """
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
)
logger = logging.getLogger(__name__)
if not any(isinstance(h, logging.FileHandler) for h in logger.handlers):
    logger.addHandler(logging.FileHandler("sa_50_3.log"))
...
class Annealer:
    ...
    def simulated_annealing(
        ...
        logger.info(f"SA start: ε={epsilon:.4f}, TWCT={twct:.2f}")
        ...
        for i in range(1, max_iterations + 1):
            ...
            if not neighbors:
                ...
                if temperature < min_temp:
                    break
                continue
            result = self.calculate_metrics(neighbors[0])

            candidate, (cand_epsilon, cand_twct) = (neighbors[0], result)
            ...
            if cand_twct < best_twct:
                best, best_epsilon, best_twct = candidate, cand_epsilon, cand_twct
                logger.info(
                    f"[Iter {i}] UPDATE best: ε={best_epsilon:.4f}, TWCT={best_twct:.2f}"
                )
...
def run_sa_for_schedule(args):
    ...
    annealer = Annealer(epsilon_threshold=0.7, params=params)
    ...
    logger.info(
        f"SA fertig für Schedule {idx}: ε={best_epsilon:.4f}, TWCT={best_twct:.2f}"
    )
    result_dict = {
        "index": idx,
...
def main():
    input_file = "new_balanced_schedules_50_3.json"
    output_file = "sa_50_3.json"
    ...
    logger.info(f"Alle Ergebnisse gespeichert in {output_file}")
