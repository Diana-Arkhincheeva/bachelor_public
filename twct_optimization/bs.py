...
if not any(isinstance(h, logging.FileHandler) for h in logger.handlers):
    logger.addHandler(logging.FileHandler("bs_50_3.log"))
    ...
    def beam_search(
        ...
        for it in range(max_iterations):
            ...
            if not all_candidates:
                logger.info("Keine weiteren Kandidaten.")
                break

            valid = [
                (sch, (eps, tw))
                for sch, (eps, tw) in all_candidates
                if eps <= self.epsilon_threshold
            ]
            # sortieren nach TWCT
            valid.sort(key=lambda x: x[1][1])
            top_k = valid[:beam_width] if valid else []

            if not top_k:
                logger.info("Keine gültigen Kandidaten mehr.")
                break
            ...
            if tw < best_twct:
                best_epsilon, best_twct, best_schedule = eps, tw, sch
                logger.info(
                    f"[Iter {it + 1}] UPDATE: ε={best_epsilon:.4f}, TWCT={best_twct:.2f}"
                )
        ...
        if output_file and valid_schedules:
            ...
            logger.info(
                f"Gespeichert {len(valid_schedules)} schedules in {output_file}"
            )
