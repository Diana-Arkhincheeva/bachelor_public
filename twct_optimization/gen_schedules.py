...
from privacyschedulingtools.total_weighted_completion_time.pup.utility_functions.parallel_machine_utilities import calculate_twct
from privacyschedulingtools.total_weighted_completion_time.pup.solver.pup_parallel import PupParallel
from privacyschedulingtools.total_weighted_completion_time.entity.scheduling_parameters import ParallelSchedulingParameters
from privacyschedulingtools.total_weighted_completion_time.entity.parallel_schedule import ParallelScheduleFactory, ParallelSchedule
from privacyschedulingtools.total_weighted_completion_time.entity.domain import IntegerDomain
from privacyschedulingtools.total_weighted_completion_time.entity.adversary.parallel_adversary import ParallelLocalWSPTAdversary

from concurrent.futures import ProcessPoolExecutor


'''Main Script für Kalendergenerierung:
Generiert schedules mit median epsilon-werte<0.5'''
...
def schedule_to_json(schedule: ParallelSchedule, epsilon: float, twct: float) -> dict:
    """Convert schedules to .json-file."""
    machines = []
...
def median_epsilon_for_schedule(
    ...
    """
    Выполняет n_attacks атак на одно расписание и возвращает медиану ε.
    """
...
class Evaluator:
    ...
    def eval_schedule(self, schedule: ParallelSchedule) -> tuple[ParallelSchedule, float, int]:
        logger.info("Start")
...
def generate_balanced_schedule(
        ...
        params: ParallelSchedulingParameters,
        max_attempts: int = 10,
        json_filename: str = "new_balanced_schedules_20_2.json"
):
    ...
    schedules = (factory.generate_random_schedule_with_dispatching_rule() for _ in range(max_attempts))
    ...
    with ProcessPoolExecutor() as executor:
        for schedule, epsilon, twct in executor.map(evaluator.eval_schedule, schedules):
            if epsilon <= epsilon_threshold:
                ...
                median_eps = median_epsilon_for_schedule(schedule, params, epsilon_threshold, n_attacks=10)
                logger.info(f"THERE IS ONE POSSIBLE SCHEDULE WITH median ε = {median_eps:.4f}")
    ...
    if valid_schedules:
        ...
        logger.info(f"{len(valid_schedules)} schedules wurden in {json_filename} gespeichert.")
    else:
...
def main():
    ...
    epsilon_threshold = 0.5  # könnte auch weniger sein.
    generate_balanced_schedule(epsilon_threshold, params, max_attempts=300)
